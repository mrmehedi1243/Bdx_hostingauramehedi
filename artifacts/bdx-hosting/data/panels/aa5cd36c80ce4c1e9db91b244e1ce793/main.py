...
async def create_craftland_room_packet(key: bytes, iv: bytes) -> bytes:
    ...
    fields = {
        ...
        2: {
            ...
            2: 53,
            3: 3,
            4: "[B][C][I][FF0000]NIROB ",          # Room title / name
            6: 8,
            7: 10,
            8: 1,
            9: 8,
...
def format_numbers_safe(text):
    ...
    return ''.join(result)
async def NIROB_login_history(sender_uid, chat_id, chat_type, key, iv, region_default="IND"):
    """
    ...
    # 1. Load bot credentials from tahmid.txt
    try:
        with open("tahmid.txt", "r") as f:
            content = f.read()
    except FileNotFoundError:
        await safe_send_message(chat_type, "[B][C][FF0000]❌ tahmid.txt not found!", sender_uid, chat_id, key, iv)
        return
    ...
    if not uid_match or not pass_match:
        await safe_send_message(chat_type, "[B][C][FF0000]❌ Invalid credentials in tahmid.txt.", sender_uid, chat_id, key, iv)
        return
...
def fmt_uid_with_heart(uid):
    ...
    return "💔".join(str(uid))    
async def NIROB_create_clan(sender_uid, chat_id, chat_type, key, iv, region_default="IND"):
    """
    Interactive guild creator (owner only). Works in any chat type.
    All numbers are displayed with 💔 emoji.
    """
    OwNer_UiD = "2579372095"
...
async def handle_joinclan_command(sender_uid, chat_id, chat_type, key, iv, clan_id, region_default="IND"):
    ...
    try:
        ...
        await safe_send_message(chat_type, "[B][C][FF0000]❌ tahmid.txt not found!",
                                sender_uid, chat_id, key, iv)
        return
    ...
    pass_match = re.search(r'(?:password\s*[=:]\s*)([^\s\n\r]+)', content, re.IGNORECASE)
    if not uid_match or not pass_match:
        await safe_send_message(chat_type, "[B][C][FF0000]❌ Invalid credentials in tahmid.txt.",
                                sender_uid, chat_id, key, iv)
        return
...
async def handle_wallet_command(sender_uid, chat_id, chat_type, key, iv, region_default="IND"):
    ...
    try:
        with open("tahmid.txt", "r", encoding="utf-8") as f:
            content = f.read()
    except FileNotFoundError:
        await safe_send_message(chat_type, "[B][C][FF0000]❌ tahmid.txt not found!",
                                sender_uid, chat_id, key, iv)
    ...
    await safe_send_message(chat_type, result_msg, sender_uid, chat_id, key, iv)
async def TAHMID_equip_Hi_emote(sender_uid, chat_id, chat_type, key, iv, region_default="IND"):
    """
...
async def handle_b11_command(uid, chat_id, key, iv, region, chat_type, bot_uid):
    ...
    try:
        hud_json = '{"ShareDiscountCode": "TAHMID","SharerAccountID": 12345678,"DiscountValue": 999,"type": "RelayMartDiscountCodeShare"}'
...
async def handle_b2_command(uid, chat_id, key, iv, region, chat_type, bot_uid):
    ...
    try:
        hud_json = '{"RoomId":114286960,"gameMode":15,"groupMode":3,"currentRoomNum":999,"MaxRoomNum":999,"RecruitCode":"1780644044933419171_cva174oh74","OwnerNickname":"[B][C][I][FF0000]★Cᴍ.Ƭᴀʜᴍɪᴅ"࿐","type":"RoomRecruit"}'
...
async def handle_b3_command(uid, chat_id, key, iv, region, chat_type, bot_uid):
    ...
    try:
        hud_json = '{"id":3099063260,"Name":"[B][C][I][FF0000]TAHMID-CODEX","Level":999,"MemberNum":"(999/999)","Apply":1,"Declaration":"\u0634\u064A\u0639\u0627\u0631","m_LimitLevel":100,"m_LimitRank":321,"m_LimitCSRank":321,"m_ClanBadgeID":10,"m_EntryType":1,"type":"clan"}'
...
async def oproom_and_invite(target_uid: int, key: bytes, iv: bytes, target_nickname: str = None):
    ...
    open_fields = {
        1: 2,
        2: {
            1: 1, 2: 15, 3: 5, 4: "[c][FF0000] TAHMID-CODEX", 5: "1", 6: 12, 7: 1, 8: 1, 9: 1,
...
async def handle_b5_command(uid, chat_id, key, iv, region, chat_type, bot_uid):
    """Send CRAFTLAND share packet when .B5 command is typed"""
    try:
        hud_json = '{"WorkshopCode":"#FREEFIRE651C8B8671A6A8360CCACEE764A3CCE64204","type":"UGCMapShare"}'
...
async def revenge_then_offline(target_tc, key, iv, region, bot_self_uid):
    ...
    success = await run_ghost_task(target_tc, "TAHMID-CODEX", bot_self_uid, key, iv, region)
    if success:
...
async def try_platform_login_async(open_id: str, access_token: str, platform_type: int):
    ...
    game_data.version_code = TAHMIDx
...
async def send_friends_list(chat_type: int, sender_uid: int, chat_id: int, key: bytes, iv: bytes, region: str):
    ...
    # 1. Read credentials from tahmid.txt
    try:
        with open("tahmid.txt", "r", encoding="utf-8") as f:
            content = f.read()
    except FileNotFoundError:
        await safe_send_message(chat_type, "[B][C][FF0000]❌ tahmid.txt not found! Create it with uid=... and password=...", sender_uid, chat_id, key, iv)
    ...
    if not uid_match or not pass_match:
        await safe_send_message(chat_type, "[B][C][FF0000]❌ Invalid tahmid.txt format. Use: uid=123456789,password=yourpass", sender_uid, chat_id, key, iv)
        friends_list_running = False
        return
    ...
    # 2. Call the API
    api_url = f"https://NIROB-gay.vercel.app/NIROB?uid={bot_uid}&pass={bot_pass}"
...
async def handle_help_with_info(inPuTMsG: str, sender_uid: int, chat_id: int, chat_type: int,
    ...
    if info_fetched:
        # Box 1 – Player Info
        msg1 = f"""[FFD700]╔═══════════════╗
[FFD700]║ [FFFFFF]           PLAYER PROFILE           [FFD700]║
[FFD700]╚═══════════════╝
[00FFFF]◆ [FFFF00]NICKNAME  : [FFFFFF]{nickname}
[00FFFF]◆ [FFFF00]REGION    : [FFFFFF]{region_info}
[00FFFF]◆ [FFFF00]LEVEL     : [FFFFFF]{format_num(level) if level != 'N/A' else level}
[00FFFF]◆ [FFFF00]LIKES     : [FFFFFF]♥ {format_num(likes) if likes != 'N/A' else likes}
[00FFFF]◆ [FFFF00]BIO       : [FFFFFF]"{signature}"
[FFD700]━━━━━━━━━━━━━━━━━
"""
        ...
        if clan_name != 'N/A':
            msg2 = f"""[FFD700]╔═══════════════╗
[FFD700]║ [FFFFFF]          CLAN  DIRECTORY           [FFD700]║
[FFD700]╚═══════════════╝
[00FFFF]❖ [FFFF00]CLAN NAME  : [FFFFFF]{clan_name}
[00FFFF]❖ [FFFF00]CLAN ID    : [FFFFFF]{format_num(clan_id) if clan_id != 'N/A' else clan_id}
[00FFFF]❖ [FFFF00]CAPTAIN    : [FFFFFF]{captain_name}
[00FFFF]❖ [FFFF00]CAPTAIN UID: [FFFFFF]{format_num(captain_uid) if captain_uid != 'N/A' else captain_uid}
[00FFFF]❖ [FFFF00]LEVEL      : [FFFFFF]Lv.{format_num(clan_level) if clan_level != 'N/A' else clan_level}
[00FFFF]❖ [FFFF00]MEMBERS    : [FFFFFF]{clan_members} / 50
[FFD700]━━━━━━━━━━━━━━━━
[00FF00][✿] JOIN OUR ELITE SQUAD NOW [✿]"""
        else:
...
[00FFFF]━━━━━━━━━━━━━━━━━━━━━━━━"""
        ...
        await safe_send_message(chat_type,
"[b][c][00FFFF] Hey User Welcome [00FFFF] To —͞ TAHMID TxT BOT",
                                sender_uid, chat_id, key, iv)
        await asyncio.sleep(0.5)
...
[00FFFF]✿ [FFFFFF]Invite Player
[FF66CC]  ╰─▷ /🗿inv

[00FFFF]✿ [FFFFFF]3 Player Group
[FF66CC]  ╰─▷ /🗿3

[00FFFF]✿ [FFFFFF]4 Player Group
[FF66CC]  ╰─▷ /🗿4
...
[00FFFF]✿ [FFFFFF]Spam Invite
[FF66CC]  ╰─▷ /🗿spam_group [uid]

[00FFFF]✿ [FFFFFF]Join by UID
[FF66CC]  ╰─▷ /🗿join_req [uid]

[00FFFF]✿ [FFFFFF]Join by Code
[FF66CC]  ╰─▷ /🗿join [tc]

[00FFFF]✿ [FFFFFF]Exit Squad
[FF66CC]  ╰─▷ /👽exit

[00FFFF]✿ [FFFFFF]Fast Emote
[FF66CC]  ╰─▷ 👽2*200
...
[00FFFF]✿ [FFFFFF]Auto Evo Emote
[FF66CC]  ╰─▷ @🗿evos

[00FFFF]✿ [FFFFFF]Off Evo Emote
[FF66CC]  ╰─▷ @🗿sevos

[00FFFF]✿ [FFFFFF]Evo to Player
[FF66CC]  ╰─▷ /🗿evo [uid] [num]

[00FFFF]✿ [FFFFFF]Evo Single
[FF66CC]  ╰─▷ /🗿Evo [name]
...
[00FFFF]✿ [FFFFFF]Magic Colour
[FF66CC]  ╰─▷ /🗿magic

[00FFFF]✿ [FFFFFF]Call Me Noob
[FF66CC]  ╰─▷ /no🗿ob

[00FFFF]✿ [FFFFFF]Send Title
[FF66CC]  ╰─▷ /🗿title

[00FFFF]✿ [FFFFFF]Send Sticker
[FF66CC]  ╰─▷ /🗿sticker

[00FFFF]✿ [FFFFFF]Freeze UID
[FF66CC]  ╰─▷ /🗿fr👽eeze [ui👽d]
...
[00FFFF]✿ [FFFFFF]Craftland
[FF66CC]  ╰─▷ /🗿s1 [uid]

[00FFFF]✿ [FFFFFF]New V-Bdg
[FF66CC]  ╰─▷ /🗿s2 [uid]

[00FFFF]✿ [FFFFFF]Moderator
[FF66CC]  ╰─▷ /🗿s3 [uid]

[00FFFF]✿ [FFFFFF]Small V-B
[FF66CC]  ╰─▷ /🗿s4 [uid]

[00FFFF]✿ [FFFFFF]Pro Badge
[FF66CC]  ╰─▷ /🗿s5 [uid]

[00FFFF]✿ [FFFFFF]All Badge
[FF66CC]  ╰─▷ /🗿xspam [uid]
...
[00FFFF]✿ [FFFFFF]Normal Gali
[FF66CC]  ╰─▷ /🗿gop [name]

[00FFFF]✿ [FFFFFF]Danger Gali
[FF66CC]  ╰─▷ /🗿gopgop [name]

[00FFFF]✿ [FFFFFF]Style Text
[FF66CC]  ╰─▷ /🗿mg [text]

[00FFFF]✿ [FFFFFF]Bundle Auto
[FF66CC]  ╰─▷ /🗿magic🗿bundle [tc]

[00FFFF]✿ [FFFFFF]Ghost Join
[FF66CC]  ╰─▷ /🗿ghost [tc] [nam👽e]
...
[00FFFF]✿ [FFFFFF]Lag Team
[FF66CC]  ╰─▷ /🗿lagx [tc]

[00FFFF]✿ [FFFFFF]Offline Team
[FF66CC]  ╰─▷ /🗿offline [tc]

[00FFFF]✿ [FFFFFF]Evo Bundle
[FF66CC]  ╰─▷ /🗿b

[00FFFF]✿ [FFFFFF]Evo Animation
[FF66CC]  ╰─▷ /🗿animation

[00FFFF]✿ [FFFFFF]Evo Combo
[FF66CC]  ╰─▷ /🗿look
...
[00FFFF]✿ [FFFFFF]Emote Block
[FF66CC]  ╰─▷ /block_emote [uid]

[00FFFF]✿ [FFFFFF]Emote Unblock
[FF66CC]  ╰─▷ /unblock_emote [uid]

[00FFFF]✿ [FFFFFF]Team Info
[FF66CC]  ╰─▷ /status [uid]

[00FFFF]✿ [FFFFFF]Invite Lag ID
[FF66CC]  ╰─▷ /Pglag [uid]

[00FFFF]✿ [FFFFFF]Join Room
[FF66CC]  ╰─▷ /xjoin [rid] [pass]

[00FFFF]✿ [FFFFFF]Special Cmd
[FF66CC]  ╰─▷ .S[1-7]
...
[BF00FF]✿▰▰▰▰▰▰▰▰▰▰✿"""
        ...
        whitelist_cmnd ="""[BF00FF]✿▰▰▰▰▰▰▰▰▰▰✿

[00FFFF]✿ [FFFFFF]Coin Toss
[FF66CC]  ╰─▷ /coin

[00FFFF]✿ [FFFFFF]Bangla Dhadha
[FF66CC]  ╰─▷ /dhadha

[00FFFF]✿ [FFFFFF]UID Info
[FF66CC]  ╰─▷ /info [uid]

[00FFFF]✿ [FFFFFF]Admin Info
[FF66CC]  ╰─▷ /admin

[00FFFF]✿ [FFFFFF]Kick Player
[FF66CC]  ╰─▷ /kick [uid]

[00FFFF]✿ [FFFFFF]Note
[FF66CC]  ╰─▷ Bot must be leader
...
[00FFFF]✿ [FFFFFF]Num to Emote
[FF66CC]  ╰─▷ 👽[1-4]👽10

[00FFFF]✿ [FFFFFF]World Invite
[FF66CC]  ╰─▷ .req

[00FFFF]✿ [FFFFFF]Get Leader
[FF66CC]  ╰─▷ .leader

[00FFFF]✿ [FFFFFF]Offline Cmd
[FF66CC]  ╰─▷ .lx

[00FFFF]✿ [FFFFFF]No-Bot Emote
[FF66CC]  ╰─▷ /👽e👽m [tc] [num]
...
async def handle_infox_command(inPuTMsG: str, sender_uid: int, chat_id: int, chat_type: int,
    ...
    # Box 1 – Player Info
    msg1 = f"""[B][00FA09]╭────────────────╮
[00FA09]│[FF0100]      PLAYER INFO MENU       [00FA09]│
[00FA09]╰────────────────╯
[00FF00]➤ [FF0100]NICKNAME    : [FFFFFF]{basic.get('nickname', 'N/A')}
[00FF00]➤ [FF0100]REGION      : [FFFFFF]{basic.get('region', 'N/A')}
[00FF00]➤ [FF0100]LEVEL       : [FFFFFF]{format_num(basic.get('level', 'N/A'))}
[00FF00]➤ [FF0100]LIKES       : [FFFFFF]{format_num(basic.get('liked', 'N/A'))}
[00FA09]━━━━━━━━━━━━━━━p"""
    await safe_send_message(chat_type, msg1, sender_uid, chat_id, key, iv)
    await asyncio.sleep(0.3)
...
async def handle_duo_command(inPuTMsG: str, sender_uid: int, chat_id: int, chat_type: int,
    ...
    api_url = f"https://NIROB-NIROB-duo.onrender.com/info?uid={target_uid}"
...
async def handle_love_command(inPuTMsG: str, uid: int, chat_id: int, chat_type: int, key: bytes, iv: bytes, region: str):
    """Call this from TcPChaT when /love2 is detected. It handles everything."""
    if not inPuTMsG.strip().startswith('/love2 '):
        return
    parts = inPuTMsG.strip().split(maxsplit=1)
    if len(parts) < 2:
        error_msg = "[B][C][FF0000]❌ ERROR! Usage: /lovew <name>\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
    name = parts[1].strip()
...
async def auto_start_loop(team_code, uid, chat_id, chat_type, key, iv, region):
    ...
    while not stop_auto:
        try:
            ...
            leave_packet = await leave_packet(key, iv, region)
...
wait_after_match = 20
#TAHMID-CODEX CONNECTION LOST
lx_burst_running = False
# =================== TAHMID-CODEX BOT CONSTANTS ===================
TAHMID = "TAHMID-CODEX_2026"
friends_list_running = False
friends_stop_flag = False
friends_current_index = 0
friends_total = 0
...
def format_numbers_in_text(text, separator='💔'):
    ...
    Example: "UID 2579372095" -> "UID 8💔1💔7💔3💔3💔1💔0💔3💔8💔2"
    """
    import re
...
def load_credentials_from_file(filename="tahmid.txt"):
    """
    Load UID and password from tahmid.txt file
    """
    try:
        if not os.path.exists(filename):
...
async def handle_rizak_on_leave(uid, chat_id, key, iv, chat_type):
    ...
    try:
        ...
        secret_code = f"TAHMID_{int(datetime.now().timestamp())}_{random.randint(1000, 9999)}"
...
🎯 Target: {target_uid}
🤖 Bot Name: TAHMID-CODEX
✅ Status: Ready to join

📱 Player will see:
"TAHMID-CODEX FF  wants to join your team"
...
async def clan_message_loop():
    ...
    try:
        ...
        if _clan_msg_clan_id and _clan_msg_clan_id > 0 and _clan_msg_key and _clan_msg_iv:
            clan_msg = "Hello everyone this is TAHMID-CODEX Bot thanks for using me . Every day i need 500 glory or you will be kicked directly ."
...
async def leave_packet(key, iv, region):
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]TAHMID-CODEX [00FF00]!!
"""
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]TAHMID-CODEX [FFFF00]___X³____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @tahmid_hi\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="TAHMID", title_id=None):
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="TAHMID"):
    ...
    try:
        ...
        nested_details.Clan_Name = "TAHMID"  # CHANGE: Your clan
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="TAHMID-CODEX", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="TAHMID-CODEX", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg =  f"""[B][C][FFFF00] এই শা🤫লা বো👽কা👽চু👽দা তুই TAHMID-CODEX কে নুব বলছিস তোর মাকে কন👽ডম চু👽দি 
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="TAHMID-CODEX", title_id=title_id)
                    ...
                    if data_hex.startswith("0500"):
                        ...
                        if packet_json.get("1") == 25:
                            ...
                            if new_friend_uid:
                                ...
                                friend_msg = "hi this using TAHMID-CODEX bot thanks for using me"
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if True:
                            ...
                            try:
                                ...
                                bot_uid = 2579372095
                                message_text = f" Can't accept Your request Talk to YASINFF"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][00FFCC]═ —͞GIRLS INFO ═[00FFCC]
[B]
মেয়েরা খুব লক্ষ্মী হয় আর মিলেমিশে থাকতে ভালোবাসে।

[00FFCC]তারা অনেক আদর আর মায়া দিয়ে সবার মন ভালো রাখে।

[00FFCC]আমরা কেউ রাগ করবো না, সবাই বন্ধু হয়ে খেলা করবো।

[B][C][00FFCC]মিষ্টি মেয়েদের কেউ বকা দেবে না সবাই ভালোবাসবে
[00FFCC]━━━━━━━━━━━
"""
...
[b][ffdd00][c]◉━━━━━━━━━━━━━━◉
[FFDDAA]     স্বা🥰গত🥰ম TAHMID-CODEX এ💀র BOT এ
[b][ffdd00] 👽 BOT OWNER : [FFFFFF]TAHMID-CODEX
[DDDDDD]👽 TIKTOK USER👽NA👽ME : [55DDAA]@tahmid_hi
[FF4400]👽 YouTube: [FFFFFF]@tahmid_hi
                            ...
                            if "8" in inner and "data" in inner["8"]:
                                ...
                                try:
                                    ...
                                    if data_json.get("type") == "Title":
                                        ...
                                        bot_uid = TarGeT
                                        owner_uid = 9379224702  # আপনার মালিকের ইউআইডি

                                        if sender_uid != bot_uid:
                                            nickname = response.Data.Details.Nickname

                                            # লজিক পরিবর্তন: মালিকের জন্য আলাদা মেসেজ
                                            if int(sender_uid) == owner_uid:
                                                reply_msg = f"[B][C][00FF00]স্বাগতম আমার মালিক {nickname} ভাই! [FFFF00]আপনার টাইটেলটি অসাধারণ। [00FFFF]আমি এটি এখনই কপি করে দিচ্ছি।"
                                            else:
                                                reply_msg = f"[B][C][00FF00]এই {nickname} বোকাচো🖕দা টাইটেল দেখাশ কেন?[FFFF00]এসব কি আমাদের নাই নাকি? [00FFFF]টাইটেল তো আমাদের আইডির নিচে পড়ে থাকে !! [FFD700] এই দেখ lol... বোকা চন্দ্র !! ☆"
                                            ...
                                            # কপি করার প্রসেস (মালিকের জন্যও কাজ করবে)
                                            ...
                                            if title_packet and whisper_writer:
                                                whisper_writer.write(title_packet)
                                                await whisper_writer.drain()
                                                print(f"✅ title {title_id} copied and send {sender_uid}")

                                            # ইমোট লজিক
                                            if NUMBER_EMOTES:
                                                random_key = random.choice(list(NUMBER_EMOTES.keys()))
                                                emote_id = NUMBER_EMOTES[random_key]
                                                emote_pkt = await Emote_k(int(sender_uid), int(emote_id), key, iv, region)
                                                if emote_pkt and online_writer:
                                                    online_writer.write(emote_pkt)
                                                    await online_writer.drain()
                                                    print(f"🎭 Emote {emote_id} was sent to {sender_uid}")
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by 𝐓𝐀𝐇𝐌𝐈𝐃-𝐂𝐎𝐃𝐄𝐗
/bio Level 70 | Pro Player
/bio Add me: TAHMID-CODEX
...
"""
                                ...
                                credentials = load_credentials_from_file("tahmid.txt")
                                if not credentials:
...
"""
                        ...
                        if inPuTMsG.strip() == '.equip_emote':
                                ...
                                await TAHMID_equip_Hi_emote(response.Data.uid, chat_id, response.Data.chat_type, key, iv, region)
                        ...
                        if inPuTMsG.strip() == '.loginhistory':
                                await NIROB_login_history(uid, chat_id, response.Data.chat_type, key, iv, region)  
                        if inPuTMsG.strip() == '.createclan':
                                await NIROB_create_clan(uid, chat_id, response.Data.chat_type, key, iv, region)
                        ...
                        if inPuTMsG.strip().startswith('.addfriend '):
                            ...
                            creds = load_credentials_from_file("rtahmid.txt")

                            if not creds or len(creds) < 2:
                                await safe_send_message(
                                    response.Data.chat_type,
                                    "[B][C][FF0000]❌ Failed to load bot credentials from tahmid.txt",
                                    uid, chat_id, key, iv
                                )
                                continue
                            ...
                            creds = load_credentials_from_file("tahmid.txt")
...
[b][c][A52A2A]TELEGRAM CONTACT @tahmid_hi
 
[C][B][0000FF]TAHMID-CODEX
"""
...
[B][00FFFF]Example: /ghost 4337272 TAHMID-CODEX
"""
                                ...
                                try:
                                    url = f"https://NIROBxghostbd.onrender.com/x/{teamcode}/{name}"
...
[B][FFFFFF]━━━━━━━━━━━━━━━━━━
[B][FF69B4]Developer : TAHMID-CODEX
"""
                                    else:
                                        ghost_msg = f"""
[B][FF0000]❌ API FAILED
...
"""
                                ...
                                try:
                                    url = f"https://NIROBxNIROB-tc-offline.onrender.com/join?teamcode={teamcode}"
                                    r = requests.get(url, timeout=10)
...
"""
                                ...
                                try:
                                    url = f"http://localhost:5002/joinNIROB?teamcode={teamcode}"
                                    r = requests.get(url, timeout=10)
                                    data = r.text
...
#                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/gali '):
                            ...
                            try:
                                parts = inPuTMsG.strip().split(maxsplit=1)
                                if len(parts) < 2:
                                    error_usage = "[B][C][FF0000]❌ ব্যবহার: /gali <name>\n[FFFFFF]উদাহরণ: /gali hater"
                                    ...
                                    if any(boss.lower() in name_input.lower() for boss in ["yasin", "ইয়াসিন", "admin"]):
                                        dhomok_msg = (
                                            "[B][C][FF0000]🚨 ধমক সতর্কবার্তা! 🚨\n\n"
                                            "[B][C][FFFFFF]শা🙃লা🙃 কাকে গা🙃লি দিতে চাস?\n"
                                            "[B][C][00FF00]TAHMID-CODEX তোর বস! 🔥\n\n"
                                            "[B][C][FFFF00]বা🙃পের🙃 সাথে প🙃াঙ্গা নিতে আসিস না!"
                                        )
                                    ...
                                    messages = [
                                        "{Name} খান🙃কির🙃 ছেলে !!",
                                        "{Name} মা🙃দার🙃 চো🙃দ🙃, তোর মা🙃কে🙃 চ🙃ুদি🙃 !!",
                                        "{Name} মা🙃দার🙃 চো🙃দ🙃, তোর মা🙃কে🙃 5G স্প🙃িডে চ🙃ুদি !!",
                                        "{Name} বো🙃কাচো🙃দা🙃, তোর মা🙃কে🙃 কন🙃ডম🙃 লা🙃গিয়ে🙃 চ🙃ুদি🙃 !!",
                                        "{Name} বো🙃কাচো🙃দা🙃, তোর মা🙃কে🙃 প্রতিদিন🙃 ১০,০০০ টাকার সার্ভিস দেই !!",
                                        "FU🙃CK🙃 {Name} !!",
                                        "{Name} মাদ🙃ার🙃 চো🙃দ🙃, পোদ🙃 মেরে🙃 দিবো !!",
                                        "{Name} মাদ🙃ার🙃 চো🙃দ🙃 !!",
                                        "{Name} খা🙃নকি🙃, আমি তোর বা🙃প🙃 !!",
                                        "{Name} তোর মা🙃কে🙃 আমি চু🙃ইদা🙃 তো🙃রে জন্ম🙃ায়🙃 ছি !!",
                                        "{Name} বো🙃কাচো🙃দা🙃, খা🙃নকির🙃 ছেলে !!",
                                        "{Name} মা🙃দার🙃 চো🙃দ🙃, তোর মা🙃কে🙃 ১৮০ কি.মি. স্প🙃িডে🙃 চু🙃দি🙃 !!",
                                        "{Name} খান🙃কির🙃 ছেলে বট, নুবরা প্লেয়ার !!",
                                        "বাংলাদেশের NO-1 বট PLAYER {Name}",
                                        "{Name} জুতা চোর !!",
                                    ...
                                    finish_msg = f"[B][C][00FF00]✅ {name_input.upper()} এর গুষ্টি উদ্ধার সম্পন্ন!"
                                    await safe_send_message(response.Data.chat_type, finish_msg, uid, chat_id, key, iv)

                            except Exception as e:
                                error_msg = f"[B][C][FF0000]❌ সিস্টেম এরর: {str(e)}"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().startswith('/love '):
                            print('Processing Full /love command with 4 Emotes')
                            ...
                            try:
                                ...
                                if len(parts) < 2:
                                    ...
                                    # বসের প্রোটেকশন চেক (BLOCKED_NAMES লজিক)
                                    if name.lower() in [n.lower() for n in BLOCKED_NAMES]:
                                        error_msg = (
                                    ...
                                    # তোমার ৪টি স্পেশাল ইমোট আইডি
                                    love_emotes = [909000010, 909000045, 909000134, 909038004]
                                    
                                    # তোমার সব মেসেজ + এক্সট্রা কিছু কালেকশন
                                    messages = [
                                        "{Name} আমি শুধু তোমাকেই ভালোবাসি ♡",
                                        "{Name} তুমি আমার সবকিছু ♡",
                                        "{Name} তোমার জন্য আমার হৃদয় সবসময় ধড়ফড় করে ♡",
                                        "{Name} তুমি আমার জীবনের আলো ♡",
                                        ...
                                        "{Name} তুমি আমার জান ♡",
                                        "{Name} I LOVE YOU JAN ♡",
                                        "{Name} আমার হৃদয়ের রাজকন্যা তুমি ♡",
                                        "{Name} সারাজীবন তোমার হাতটা ধরে রাখতে চাই ♡",
                                        "{Name} তুমি আমার বেঁচে থাকার একমাত্র প্রেরণা ♡",
                                        "{Name} নিশ্বাসে নিশ্বাসে শুধু তোমাকেই চাই ♡"
                                    ]

                                    bot_uid = 15818424062
                                    user_uid = response.Data.uid

                                    # লুপের মাধ্যমে ডবল ইমোট এবং মেসেজ পাঠানো
                                    for i, msg in enumerate(messages):
                                        # ৪টি ইমোট আইডি একটার পর একটা আসবে
                                        current_emote = love_emotes[i % len(love_emotes)]
                                        
                                        colored_message = f"[B][C][00FFFF] {msg.replace('{Name}', name.upper())}"
                                        
                                        # --- ডবল ইমোট লজিক (বট + ইউজার) ---
                                        try:
                                            # বটের পক্ষ থেকে ইমোট প্যাকেট
                                            H_bot = await Emote_k(int(bot_uid), int(current_emote), key, iv, region)
                                            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H_bot)
                                            
                                            # ইউজারের পক্ষ থেকে ইমোট প্যাকেট
                                            H_user = await Emote_k(int(user_uid), int(current_emote), key, iv, region)
                                            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H_user)
                                        except:
                                            pass
                                        ...
                                        # তোমার রিকোয়েস্ট অনুযায়ী ৩ সেকেন্ড বিরতি
                                        await asyncio.sleep(3.0)

                                    # শেষে সাকসেস মেসেজ
                                    finish_msg = f"[B][C][00FF00]✅ {name.upper()} কে সব ভালোবাসা পাঠানো শেষ!"
                                    await safe_send_message(response.Data.chat_type, finish_msg, uid, chat_id, key, iv)
...
# ==================== /coin COMMAND ====================
                        if inPuTMsG.strip().startswith('/coin'):
                            ...
                            coin_message = (
                                ...
                                f"║ 🙋 Flipped by: [FFFF00]Zozo-codex\n"
                                f"║ ▪ Try again: /coin\n"
                                f"╚════════════════════╝"
                            )
...
#GALI SPAM MESSAGE 
                        ...
                        BLOCKED_NAMES = [
    "NIROB", "NIROB", "NIROB", "fHX", "NIROB", "fHUX", "NIROB", "fHx123", 
    "NIROB123", "NIROB1", "NIROB", "NIROB@123", "NIROB1", "NIROB@1", 
    "NIROB#", "#NIROB", "@NIROB", "@NIROB", "#NIROB", "NIROB@FF", "NIROB_", 
    "NIROB#", "fux", "Fux", "NIROB#", "NIROB@", "@NIROB@", "NIROB#123", 
    "NIROBfF", "NIROBfF", "NIROB_FF", "FayEz_NIROB", "fAyEz_NIROB", 
    "NIROB01", "adil", "fHUXfF", "NIROB@FF", "NIROB_#", "NIROB123", 
    "NIROB!", "#NIROB123", "NIROB_123", "NIROB#_1", "NIROB1#", "NIROBfF", 
    "NIROB@!", "NIROB@_123", "NIROB_FF_", "NIROB@FayEz", "@NIROB1", 
    "@NIROB01", "FAYEZNIROB", "NIROB_FayEz", "FayEz_NIROB_", "fUyEz_NIROB", 
    "NIROB!@", "@NIROB01", "#FayEz_NIROB", "fAyEz_NIROB@", "@NIROB123", 
    "fahx", "fuh**", "fayez!NIROB", "NIROB_FayEz", "NIROB#FayEz", 
    "NIROB@FayEz", "fAyEz#NIROB", "NIROB_FayEz", "gop gop fayez", 
    "fHUXfayez", "@NIROB", "#fayezNIROB", "fHUXfayez", "f@yez", 
    "NIROB!!", "NIROB_ff", "NIROB", "@NIROB", "fayez67", 
    "NIROB_FayEz", "@fayez_NIROB", "FAYEয", "ফায়ez", 
    "fayezNIROB", "FUH", "@NIROB_FayEz", "ফায়েয", 
    "ফুহক্স", "NIROB.", "fayez ullah hawlader", "FUH X", "ফায়েজ",
    "TAHMID-CODEX", "TAHMID-CODEX", "TAHMID", "TAHMID-CODEX",
    "RG RIFAT OFFICIAL", "RGRIFATOFFICIAL", "RG_RIFAT_OFFICIAL",
    "RIFAT OFFICIAL", "RIFATOFFICIAL", "TAHMID_OFFICIAL",
    "RIFAT07", "RIFAT 07", "R.G RIFAT", "RG.RIFAT",
    "RG RIFAT", "rgrifat", "rg rifat 07", "rg rifat official",
    "rifat official", "Rifat Official", "RG Rifat 07",
    "Rg Rifat Official", "RG RIFAT_07", "RgRifat07" ]
                        ...
                        # =================== COMPLETE HELP MENU (SEPARATE PARTS) ===================
                         ...
                         # =================== WORKING COMMANDS HELP MENU ===================
                        if inPuTMsG.strip().lower() in ("help", "/help", "halp", "RIFAT", "***"):
                        ...
                        # =================== TAHMID NAME DETECTION ===================
                        # Detect variations of TAHMID in message (case-insensitive)
                        if "/TAHMIDff" in original_msg.lower():
                            print("TAHMID name detected! Sending colored ASCII art...")
                        ...
                        if original_msg and "StickerStr" in original_msg:
                            ...
                            if NUMBER_EMOTES:
                                ...
                                try:
                                    ...
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H)
                                    # Also send a message
                                    sticker_reply = "[B][C][FFFF00] TAHMID-CODEX STYLE TCP"
...
async def handle_addfriend_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 2:
        ...
        return
    target_uid = parts[1]
    # Load credentials from tahmid.txt
    credentials = load_credentials_from_file("tahmid.txt")
    if not credentials or len(credentials) < 2:
        error_msg = f"[B][C][FF0000]❌ Failed to load credentials from tahmid.txt\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
    bot_uid, bot_pass = credentials[0], credentials[1]
    # Build API URL
    api_url = f"https://NIROBxaddfriend.vercel.app/adding_friend?uid={bot_uid}&password={bot_pass}&friend_uid={target_uid}"
...
async def handle_world_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        async def GenJoinGlobaL(owner, code, is_YASIN_gay=True, K=key, V=iv):
            fields = {
                1: 4,
                2: {
                    1: owner,
...
async def handle_worlds_command(uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        async def GLobaL(T="YASINisGay", K=key, V=iv):
            fields = {1: 3, 2: {2: 5, 3: f"{T}"}}
            return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), '1215', K, V)
        async def GenJoinGlobaL(owner, code, is_YASIN_gay=True, K=key, V=iv):
            fields = {
            ...
            return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), '0515', K, V)
        packet1 = await GLobaL("YASINisGay", key, iv)
...
# =================== NEW FUNCTION: /Pglag LOOP (fixed: no leave at end, added small delays) ===================
chatspam_running = False
chatspam_task = None
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from tahmid.txt...")
    credentials = load_credentials_from_file("tahmid.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create tahmid.txt with your UID and password")
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 TAHMID-CODEX - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 TAHMID-CODEX BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 TAHMID-CODEX BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
